...
## Purpose
Main plugin directory containing all agent definitions, commands, and skills for the autopsy Claude Code plugin. Implements an orchestrator-worker pattern for autonomous code review and architecture assessment, producing REVIEW_REPORT.md and ARCHITECTURE_REPORT.md.
...
## Data Flow
User invokes `/autopsy:full-review` → orchestrator creates `.autopsy/` → launches discovery agent → reads batch plan → launches 5 review agents + architect + researcher (7 parallel for first batch) → verifies outputs → launches both synthesizers in parallel (code review + architecture) → verifies REVIEW_REPORT.md + ARCHITECTURE_REPORT.md → prints summary
...
- File-based IPC: All agents write to `.autopsy/`, orchestrator reads from disk
...
## Configuration
No environment variables. Plugin reads repo metadata (package.json, pyproject.toml, etc.) during discovery phase. Working directory `.autopsy/` is created on-demand and should be gitignored.
...
## Testing
- Tests: Manual testing via `claude --plugin-dir ./autopsy`
- Run: Load plugin, invoke `/autopsy:full-review` on a test repo
