...
Discovery  Review    Synthesis  (maintain-docs)
    ...
    └──────────┬────────┘
               │
       .autopsy/ (disk-based coordination)
```
...
| Directory | Purpose |
|-----------|---------|
| `autopsy/` | Main plugin root (agents, commands, skills, manifest) |
| `conductor/` | Project management (tracks, workflow, product specs) |
| `architect/` | Architecture design docs (ADRs, interfaces, patterns) |
| `.autopsy/` | Working directory created during review runs (gitignored) |
...
- File-based coordination via `.autopsy/` directory (not TaskOutput)
