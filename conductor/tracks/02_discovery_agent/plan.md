...
- [x] **1.1** Write `autopsy/agents/discovery.md` with YAML frontmatter (name, description, tools) replacing the placeholder
...
- [x] **1.4** Write Step 3 (AGENTS.md Generation): trigger rules (3+ code files), subdirectory template (under 60 lines), root template (under 80 lines), companion CLAUDE.md creation, idempotency logic with `<!-- generated by autopsy -->` marker
- [x] **1.5** Write Step 4 (Discovery Profile): `.autopsy/discovery.md` template with repo stats, architecture diagram, module summary table, risk areas
- [x] **1.6** Write Step 5 (Batch Plan Generation): `.autopsy/batch-plan.md` template matching interface contract, directory-based grouping rules, batch sizing constraints (≤50 files / ≤5000 LOC), pattern-based risk scoring (auth, SQL, crypto, payments, user input)
