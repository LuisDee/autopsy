...
## Purpose
Command definitions for user-facing slash commands in the autopsy plugin. Commands orchestrate agents and manage the review lifecycle.
...
## Data Flow
`full-review.md`: User invokes → creates .autopsy/ → launches discovery (foreground) → reads batch-plan.md → first batch: launches 7 agents (5 review + architect + researcher, parallel foreground) → subsequent batches: 5 review agents → verifies all outputs → launches both synthesizers in parallel (code review + architecture) → verifies REVIEW_REPORT.md + ARCHITECTURE_REPORT.md → prints summary
...
- Run: `/autopsy:full-review` or `/autopsy:maintain-docs`
