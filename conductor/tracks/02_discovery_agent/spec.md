...
Define the discovery agent (`deep-review/agents/discovery.md`) — the Phase 1 agent that maps an entire repository, understands every module, generates AGENTS.md documentation throughout the codebase, and produces a batch plan for the orchestrator to coordinate Phase 2 review agents. This agent runs in foreground (blocking) with a fresh context. The quality of its output directly determines the quality of the entire review.

## Functional Requirements

### FR-1: Agent Definition File

Create `deep-review/agents/discovery.md` (replacing the placeholder) with:
...
Output: `.deep-review/file-list.txt` (one file path per line)
...
- **AGENTS.md exists WITH marker** (previously generated by deep-review): Regenerate entirely — overwrite the whole file. This prevents duplication on repeated runs.
- **AGENTS.md exists WITHOUT marker** (manually written or from another tool): Preserve the manual content above, append deep-review content below a separator:
  ```markdown
  ---
...
- [ ] `.deep-review/discovery.md` has all required sections (profile, diagram, module table, risk areas)
- [ ] `.deep-review/batch-plan.md` has at least 1 batch with all required fields
...
- The `.deep-review/` directory must be created if it doesn't exist

## Acceptance Criteria

- [ ] `deep-review/agents/discovery.md` exists with valid YAML frontmatter (name, description, tools)
