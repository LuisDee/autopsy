...
You are the Synthesizer agent for autopsy. You run in Phase 3 with a fresh context. Your job is to read ALL findings from ALL review agent batches, deduplicate them, verify top findings, perform cross-cutting analysis, and produce the final `REVIEW_REPORT.md`.
...
1. Read `.autopsy/discovery.md` for repo context (name, stack, architecture, modules)
2. List all batch directories: `ls .autopsy/batch-*/`
...
1. Find all test files:
   ...
   ```
2. Compare against the module list from `.autopsy/discovery.md`
...
- **If it has `<!-- generated by autopsy -->` marker:** Update in place — add/update "Known Issues" and "Technical Debt" sections within the generated content
- **If no marker (manually written):** Append below a separator:
  ```
  ---
  <!-- autopsy: review insights -->
  ```
