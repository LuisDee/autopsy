...
Define the synthesizer agent (`deep-review/agents/synthesizer.md`) — the Phase 3 agent that reads all findings from all batches, deduplicates them at three levels, spot-checks top findings, performs cross-cutting analysis, updates root AGENTS.md, and produces the final `REVIEW_REPORT.md`. This agent runs in foreground (blocking) with a fresh context.
...
Create `deep-review/agents/synthesizer.md` (replacing placeholder) with:
...
- Respect the idempotency marker (`<!-- generated by deep-review -->`) — if present, update in place; if not, append below a separator
...
- [ ] `deep-review/agents/synthesizer.md` exists with valid YAML frontmatter
