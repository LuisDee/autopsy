...
Two supporting components: the `/autopsy:maintain-docs` command that keeps AGENTS.md files current after code changes, and the `codebase-documentation` passive skill that auto-triggers during normal development to remind Claude to read/update AGENTS.md files. Together they ensure the documentation generated by the full review stays accurate over time.
...
- Passive skill must NOT trigger during autopsy operations
...
4. **Deep-review detection:** Check for `.autopsy/` directory existence vs check for a specific lock file?
